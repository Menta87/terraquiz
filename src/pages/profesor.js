...
export default function ProfesorPage() {
  ...
  async function createTest() {
    ...
    if (selectedChapters.length === 0) return setError('Alege cel puțin un capitol');
    
    setCreating(true);
    const { data: { session } } = await supabase.auth.getSession();
    
    // Iau întrebări random din capitolele alese
  ...
  if (!isTeacher) {
    return (
      <div className="container" style={{padding:'4rem 1.5rem', textAlign:'center', maxWidth:'600px'}}>
        ...
        <p style={{color:'#64748b', fontSize:'1.1rem'}}>Această secțiune este dedicată profesorilor. Dacă ești cadru didactic și dorești acces, contactează-ne la robert_menta@yahoo.com</p>
        <Link href="/" style={{display:'inline-block', marginTop:'1.5rem', padding:'0.85rem 1.5rem', background:'linear-gradient(135deg, #8b5cf6, #6d28d9)', color:'white', borderRadius:'10px', textDecoration:'none', fontWeight:700}}>← Înapoi la pagina principală</Link>
      </div>
    );
  }
