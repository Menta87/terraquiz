...
export default function ExercitiuComparatie({
  enunt,                    // "Precizați 3 deosebiri între clima Belgiei și clima Greciei"
  raspunsExemplu,           // Array cu 3 deosebiri ca exemplu de la barem
  punctajMaxim = 6,
  onRaspuns,
}) {
  const [deosebiri, setDeosebiri] = useState(['', '', '']);
  const [verificat, setVerificat] = useState(false);
  const [autoEval, setAutoEval] = useState([null, null, null]); // null/'corect'/'partial'/'gresit'
  ...
  return (
    <div style={{
      ...
      <div style={{fontSize:'0.85rem', color:'#64748b', marginBottom:'0.75rem', fontStyle:'italic'}}>
        💡 Scrie 3 deosebiri (la BAC, fiecare deosebire valorează {punctajMaxim/3} puncte)
      </div>
      ...
      {[0, 1, 2].map((index) => (
        <div key={index} style={{marginBottom:'0.75rem'}}>
          ...
          {/* Auto-evaluare apare după ce e verificat */}
          {verificat && (
