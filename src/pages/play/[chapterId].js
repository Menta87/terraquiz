...
// Map component - loads Leaflet dynamically (only on client)
function HintMap({ lat, lng, radius }) {
  useEffect(() => {
    ...
    function initMap() {
      ...
      // Add a vague circle (no exact marker, just a hint area)
    ...
    <div className="quiz-container">
      ...
      <div className="question-card">
        ...
        {hasMap ? (
          <HintMap 
            key={`map-${currentIdx}`}
            lat={parseFloat(q.map_lat)} 
            lng={parseFloat(q.map_lng)} 
            radius={parseFloat(q.map_radius) || 500000}
          />
        ) : q.image_description ? (
